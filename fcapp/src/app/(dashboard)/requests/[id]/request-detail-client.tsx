...
import type { Request, User, Dealership, RequestMessage, RequestAttachment, RequestStatus } from "@prisma/client";
...
export function RequestDetailClient({ request: initialRequest, currentUser, contractors }: RequestDetailClientProps) {
  const [request, setRequest] = useState(initialRequest);
  const [isUpdating, setIsUpdating] = useState(false);
  const [selectedContractor, setSelectedContractor] = useState(request.assignedToId || "");

  const canEdit = request.requestedById === currentUser.id || currentUser.role === "agency_admin";
  const canAssign = currentUser.role === "manager" || currentUser.role === "agency_admin";
  // Anyone who can view the request can update its status
    ...
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          ...
          {canAssign && (
            <div className="bg-white border border-gray-200 rounded-lg p-6">
              <h3 className="text-sm font-semibold text-gray-900 mb-3">Assign Contractor</h3>
              {request.assignedTo ? (
                <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                  <div className="text-xs text-blue-600 mb-1">Currently assigned to</div>
                ...
                <p className="text-sm text-gray-600 mb-4">No contractor assigned yet</p>
              )}
              ...
              {contractors.length > 0 ? (
                <div className="space-y-3">
                  ...
                  <Button
                    ...
                    disabled={!selectedContractor || isUpdating}
                    loading={isUpdating}
                    className="w-full"
                  >
                    Assign
                ...
                <p className="text-sm text-gray-500">No contractors available for this department</p>
              )}
            </div>
          )}
